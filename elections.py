...
class Election:
    ...
    def read_results(self, input_stream: TextIO) -> None:
        ...
        for line in input_stream:
            ...
            riding = res[RIDING].replace('"', '')
            vote = int(res[VOTES])
            party = res[PARTY].replace('"', '')
    ...
    def riding_winners(self, riding: str) -> List[str]:
        ...
        """
        winners = []
        party = self._results[riding]
        vote_most = max(party.values())
        for pa in party:
            if party[pa] == vote_most:
                winners.append(pa)
        return winners
    ...
    def party_seats(self) -> Dict[str, int]:
        ...
        for ride in self._ridings:
            party = self._results[ride]
            for p in party:
                seat[p] = 0

        for r in self._ridings:
            for i in self.riding_winners(r):
                seat[i] += 1
        return seat
    ...
    def election_winners(self) -> List[str]:
        ...
        vote_res = list(self.party_seats())
        vote_most = max(self.party_seats())
...
class Jurisdiction:
    ...
    def read_results(self, year: int, month: int, day: int,
        ...
        date_res = date(year, month, day)
        if date_res in self._elections:
            self._elections[date_res].read_results(input_stream)
        else:
            elc = Election(date_res)
            elc.read_results(input_stream)
            self._elections[date_res] = elc
    ...
    def party_wins(self, party: str) -> List[date]:
        ...
        for da in self._elections:
            ele = self._elections[da]
            a = ele.election_winners()
            if party in a:
                win_party.append(da)
        return win_party
    ...
    def party_history(self, party: str) -> Dict[date, float]:
        ...
        for da in self._elections:
            for i in self._elections.values():
                count = sum(i.popular_vote().values())
                got = i.popular_vote().get(party)
                percentage = got / count
                history[da] = percentage
    ...
    def riding_changes(self) -> List[Tuple[Set[str], Set[str]]]:
        ...
        """
        change = []
        date_res = list(self._elections.keys())
        date_res.sort()

        for i in range(len(date_res) - 1):
            d1 = date_res[i]
            d2 = date_res[i + 1]
            ele1 = self._elections[d1]
            ele2 = self._elections[d2]
            re1 = set(ele1.ridings_recorded())
            re2 = set(ele2.ridings_recorded())

            ch1 = re1 - re2
            ch2 = re2 - re1

            change.append((ch1, ch2))
